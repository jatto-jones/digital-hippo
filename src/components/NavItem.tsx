...
const NavItem = ({ category, handleOpen, isAnyOpen, isOpen }: NavItemProps) => {
  return (
    <div className="flex">
      ...
      {isOpen ? (
        <div className={cn('absolute inset-x-0 top-full text-sm text-muted-foreground', !isAnyOpen && 'animate-in fade-in-10 slide-in-from-top-5')}>
                        ...
                        <div className='col-span-4 col-start-1 grid grid-cols-3 gap-x-8'>
                            {category.featured.map(item => (
                                ...
                                className='group relative text-base sm:text-sm'>
                                    ...
                                    <Link href={item.href} className='mt-6 block font-medium text-gray-900 underline hover:no-underline'>
                                        {item.name}
                                    </Link>
